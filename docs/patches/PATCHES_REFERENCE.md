...
- The preview is valid when the rectangle holds exactly one clue and satisfies it, and invalid otherwise (two or more clues, wrong area, wrong shape, or overlap with another clue's patch).
- Releasing an invalid rectangle places nothing. The board shakes briefly and a text message names the reason.
...
- Drawing again from a clue that already has a patch replaces it in one undoable step and counts as a redraw. The old patch fades while the new one is drawn.
